...
#define H 64
#define C 6
int mambahar_forward(float *input)
{
    Tensor *mamba_input = tensor_zeros(B, T, H);
    for (int b = 0; b < B; b++)
    {
        for (int t = 0; t < T; t++)
        {
    ...
    Tensor *output = tensor_zeros(B, T, H);
    Mamba_Forward(model, mamba_input, output);
    Tensor *output_T = tensor_zeros(B, T, H);
    for (int b = 0; b < B; b++)
    {
        for (int t = 0; t < T; t++)
        {
