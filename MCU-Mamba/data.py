...
class SpeechCommandsWrapper(torch.utils.data.Dataset):
    ...
    def __init__(self, subset, data_dir, mfcc_transform):
    ...
    def __getitem__(self, idx):
        ...
        mfcc = self.preprocess(waveform, sample_rate)   # (T, F)
...
def load_speechcommands_data(data_dir, n_mfcc=40):
    ...
    train_ds = SpeechCommandsWrapper("training", data_dir, mfcc_transform)
    val_ds   = SpeechCommandsWrapper("validation", data_dir, mfcc_transform)
    test_ds  = SpeechCommandsWrapper("testing", data_dir, mfcc_transform)
